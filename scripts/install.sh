...
# (legacy internal mdns_publisher removed; no dynamic TXT publisher is installed)
...
while [ $# -gt 0 ]; do
	...
	--remote-retries)
		...
		;;
	--remote-backoff)
		REMOTE_BACKOFF_BASE=$2
		shift 2
		;;
	--prefix)
		PREFIX=$2
		add_pass_arg --prefix "$2"
		shift 2
		;;
	--force-config)
		FORCE_CONFIG=1
		add_pass_arg --force-config
		shift
		;;
	--no-start)
		START_SERVICE=0
		add_pass_arg --no-start
		shift
		;;
	--add-sysupgrade)
		ADD_SYSUPGRADE=1
		add_pass_arg --add-sysupgrade
		shift
		;;
	--deps-auto-yes)
		DEPS_MODE="yes"
		add_pass_arg --deps-auto-yes
		shift
		;;
	--deps-auto-no)
		DEPS_MODE="no"
		add_pass_arg --deps-auto-no
		shift
		;;
	--install-optional)
		INSTALL_OPTIONAL=1
		add_pass_arg --install-optional
		shift
		;;
	--fix-wireless)
		FIX_WIRELESS=1
		add_pass_arg --fix-wireless
		shift
		;;
	--status-wait)
		STATUS_WAIT=$2
		...
		;;
	--auto-migrate-legacy)
		AUTO_MIGRATE_LEGACY=1
		add_pass_arg --auto-migrate-legacy
		shift
		;;
	--cleanup-legacy)
		CLEANUP_LEGACY=1
		add_pass_arg --cleanup-legacy
		shift
		;;
	--cleanup-legacy-force)
		CLEANUP_LEGACY=1
		CLEANUP_LEGACY_FORCE=1
		add_pass_arg --cleanup-legacy-force
		shift
		;;
	-h | --help)
		cat <<EOF
...
if [ -n "$REMOTE_HOSTS" ]; then
	...
	# If the optional sidecar was built locally, include it in the transfer so remote installs can stage it
	# publisher removed; no extra files
	# Include sidechannel/broadcast helper scripts if present
	if [ -f "$REPO_ROOT/bin/nrsyncd_sidechannel" ]; then
		FILE_LIST="$FILE_LIST bin/nrsyncd_sidechannel"
	fi
	if [ -f "$REPO_ROOT/bin/nrsyncd_broadcast_helper" ]; then
		FILE_LIST="$FILE_LIST bin/nrsyncd_broadcast_helper"
	fi
	...
	: >"$MANIFEST_PATH" || {
		echo "[nrsyncd] ERROR: cannot create manifest at $MANIFEST_PATH" >&2
		exit 1
	}
...
copy_file() {
	...
	# Copy content
	cp "$REPO_ROOT/$src" "$tmp" 2>/dev/null || {
		echo "[nrsyncd] ERROR: copy failed for $src" >&2
		rm -f "$tmp"
		return 1
	}
...
# Optional internal publisher, if developer build placed it at build/mdns_publisher
# publisher removed
# Optional sidechannel prototype
if [ -f "$REPO_ROOT/bin/nrsyncd_sidechannel" ]; then
	copy_file bin/nrsyncd_sidechannel /usr/bin/nrsyncd_sidechannel 0755
fi
if [ -f "$REPO_ROOT/bin/nrsyncd_broadcast_helper" ]; then
	copy_file bin/nrsyncd_broadcast_helper /usr/bin/nrsyncd_broadcast_helper 0755
fi

# Config handling
if [ ! -f "$(dest /etc/config/nrsyncd)" ] || [ $FORCE_CONFIG -eq 1 ]; then
	copy_file config/nrsyncd.config /etc/config/nrsyncd 0644
else
	echo "[nrsyncd] Preserving existing /etc/config/nrsyncd (use --force-config to overwrite)"
fi
...
check_wireless_rrm() {
	...
	[ ! -f "$wcfg" ] && {
		echo "[nrsyncd] WARNING: $wcfg not found; cannot verify 802.11k/v options" >&2
		return 0
	}
	awk '
      function flush(){
...
if [ "$ADD_SYSUPGRADE" -eq 1 ]; then
	...
	# Persist sidecar if present
	# publisher removed; nothing to persist for it
